...
/* delare several global variables
 * the gfx_open function creates several X11 objects, and stores them in these globals
...
/* commenting out because it fails with g++ (works with gcc) 
 * I will look for a fix later 
 * it's not crucial; the gfx_fast_color variable is only used 
 * for shortcut purposes below in the gfx_color function
*/
/*
...
/* Change the text font that will be used by gfx_text() */
